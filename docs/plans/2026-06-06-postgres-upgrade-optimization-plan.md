...
2026-06-18 progress: R78 已补强服务器终端 fallback 契约。服务器自运行入口执行前会校验 all-domain canary 阶段与 final cutover readiness，避免旧 sequence 或缺脚本时继续执行。
...
2026-06-18 追加：R81 新增 `scripts/postgres_post_json_exit_validation.sh` 与 verifier，artifact 已生成到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r81-post-json-exit-validation/`；当前状态为 `dry_run`。该后置门禁会在 final JSON exit audit 进入 `ready_for_post_json_exit_validation` 后，滚动最终运行态、检查本机/公网 health、执行 9 个 tracked 核心域 reconcile、采集 Compose/Docker 状态，并跑 k6 S1 读负载门禁。
2026-06-18 追加：R82 新增 `scripts/postgres_apply_final_json_exit_policy.sh` 与 verifier，artifact 已生成到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r82-apply-final-json-exit-policy/`；当前状态为 `dry_run`。该脚本在服务器 sequence 通过后备份 `compose.env`，写入最终 PostgreSQL-only 运行态策略，并复跑 final JSON exit audit，作为 R81 post-exit validation 的前置实际应用步骤。
...
2026-06-18 追加：R87 新增 `scripts/postgres_archive_json_after_final_exit.py` 与 verifier，artifact 已生成到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r87-archive-json-after-final-exit/`；当前状态为 `blocked`，原因是 R86 completion status 仍为 `needs_final_exit_evidence`。该脚本只在 `postgres_only_complete` 后允许对 tracked 业务 JSON 创建 tar.gz 并移动到 quarantine，且明确排除根级敏感配置与用户账号文件。
2026-06-18 追加：R88 补齐 user/account runtime coverage。`UserService.list_user_ids()` 不再固定读取根级 `users.json`，而是通过 `user_config_runtime.list_user_ids()` 跟随 `user_config` PostgreSQL read/primary 运行态；file-only 模式仍保持 JSON 行为，PostgreSQL 列表失败只在 `MIEMIE_DATABASE_JSON_FALLBACK_READ=true` 时回退 JSON。该补口覆盖视频任务启动恢复枚举用户的最终脱 JSON 缺口；本地验证 `test_user_config_read_switch.py` 为 `8 passed`，user/config 目标回归为 `21 passed`。服务器最终 exit 仍待执行。

...
状态：已新增 `docs/adr/ADR-0003-pre-database-architecture-checkpoint.md` 作为数据库阶段前检查点，并在用户确认后接受 Compose 内 PostgreSQL 作为最终核心业务状态库。已新增 `docs/plans/2026-06-06-postgres-upgrade-optimization-plan.md`，明确 JSON 过渡、双写对账、分域读切换和最终数据库主数据源路线；第一实施域建议为视频工作室任务索引/任务状态。2026-06-07 已新增 `docs/superpowers/plans/2026-06-07-postgres-platform-upgrade-execution.md` 作为 goal 模式执行路线，并完成 preflight artifact `docs/reports/artifacts/2026-06-07-postgres-upgrade-preflight/`：本地工具链、后端关键测试、前端 typecheck/chunk 检查、服务器 SSH/Compose/health、Cloudflare health 均通过，服务器已预拉取 `postgres:16-alpine`。R1/R2 本地实现已完成：Compose PostgreSQL、database health、备份/恢复脚本和 health 测试已落地，业务读写仍默认 JSON。`video_studio_tasks`、`studio_tasks`、`projects`、media metadata 和 `project_entities` 均已完成 schema/repository、backfill/reconcile、runtime dual-write、read-switch/JSON fallback 和 PostgreSQL primary-write/JSON archive mirror；`project_entities` 覆盖角色、场景、道具、首帧、视频和风格。benchmark records 已完成图片/视频测评 dataset/suite/run 的 schema/repository boundary、backfill/reconcile 工具、runtime dual-write 和 read-switch/JSON fallback。默认运行态仍为 file-only。R29 复查显示当前 operator path 仍因 fake-IP/utun、公网 health 超时和 SSH command closed 不能安全执行服务器 rollout；下一步继续本地 benchmark primary-write 和 user/config 域迁移，服务器路径恢复后再验证/修复 PostgreSQL 容器和 health，执行 live migration/backfill/reconcile，再按 dual-write、read-switch、primary-write 逐级门禁推进。
...
6. Compose PostgreSQL R1/R2 本地实现已完成；服务器 rollout 已启动但未闭环，当前记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r1-r2-staging/`。R3 本地 schema/Alembic 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r3-local-schema/`，但 live migration 待服务器恢复。R4 本地 repository boundary 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r4-local-repository/`。R5 本地 backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r5-backfill-reconcile/`。R6 runtime dual-write feature flag 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r6-runtime-dual-write/`。R6 read switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r6-read-switch/`。R6 PostgreSQL primary write + JSON archive mirror 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r6-postgres-primary-write/`。R7 staging precheck 已尝试并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r7-staging-precheck/`，但本机 Clash TUN/fake-ip 路径导致 SSH banner/public health 证据不可用，本轮未修改服务器状态。R8 `studio_tasks` 本地 schema/repository 基础已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r8-studio-tasks-local-schema-repository/`。R9 `studio_tasks` backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r9-studio-tasks-backfill-reconcile/`。R10 `studio_tasks` runtime dual-write 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r10-studio-tasks-runtime-dual-write/`。R11 `studio_tasks` read switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r11-studio-tasks-read-switch/`。R12 `studio_tasks` PostgreSQL primary-write + JSON archive mirror 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r12-studio-tasks-primary-write/`。R13 `projects` 本地 schema/repository 基础已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r13-projects-local-schema-repository/`。R14 `projects` backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r14-projects-backfill-reconcile/`。R15 `projects` runtime dual-write 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r15-projects-runtime-dual-write/`。R16 `projects` read switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r16-projects-read-switch/`。R17 `projects` PostgreSQL primary-write + JSON archive mirror 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r17-projects-primary-write/`。R18 staging 连通性刷新已记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r18-staging-connectivity-refresh/`：当前 operator path 仍因 TUN/fake-IP/SSH 关闭而不能执行服务器 rollout，且本轮未修改服务器状态。R19 media metadata 本地 schema/repository 基础已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r19-media-metadata-local-schema-repository/`。R20 media metadata backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r20-media-metadata-backfill-reconcile/`。R21 media metadata runtime dual-write 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r21-media-metadata-runtime-dual-write/`。R22 media metadata read-switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r22-media-metadata-read-switch/`。R23 media metadata PostgreSQL primary-write + JSON archive mirror 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r23-media-metadata-primary-write/`。R24 project entities 本地 schema/repository 基础已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r24-project-entities-local-schema-repository/`。R25 project entities backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r25-project-entities-backfill-reconcile/`。R26 project entities runtime dual-write 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r26-project-entities-runtime-dual-write/`。R27 project entities read-switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r27-project-entities-read-switch/`。R28 project entities PostgreSQL primary-write + JSON archive mirror 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r28-project-entities-primary-write/`。R29 staging 连通性刷新已记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r29-staging-connectivity-refresh/`，仍不能安全执行服务器 rollout。R30 benchmark records 本地 schema/repository 基础已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r30-benchmark-records-local-schema-repository/`。R31 benchmark records backfill/reconcile 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r31-benchmark-records-backfill-reconcile/`。R32 benchmark records runtime dual-write 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r32-benchmark-records-runtime-dual-write/`。R33 benchmark records read-switch + JSON fallback 已完成并记录在 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r33-benchmark-records-read-switch/`。当前运行态仍默认 file-only。下一步：继续本地 benchmark primary-write 和 user/config 域迁移；恢复 SSH/health 后验证或修复 `postgres` 容器、`pg_isready`、`/api/health.database`、备份/恢复演练和 JSON 默认路径，再执行 live migration/backfill/reconcile、staging dual-write、staging read switch 和 staging primary-write；SSE 继续后置，不与数据库第一阶段绑定。

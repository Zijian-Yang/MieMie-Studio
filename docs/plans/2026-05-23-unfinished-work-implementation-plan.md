...
状态：已新增 `docs/adr/ADR-0003-pre-database-architecture-checkpoint.md` 作为数据库阶段前检查点，并在用户确认后接受 Compose 内 PostgreSQL 作为最终核心业务状态库。已新增 `docs/plans/2026-06-06-postgres-upgrade-optimization-plan.md`，明确 JSON 过渡、双写对账、分域读切换和最终数据库主数据源路线；第一实施域建议为视频工作室任务索引/任务状态。2026-06-07 已新增 `docs/superpowers/plans/2026-06-07-postgres-platform-upgrade-execution.md` 作为 goal 模式执行路线，并完成 preflight artifact `docs/reports/artifacts/2026-06-07-postgres-upgrade-preflight/`：本地工具链、后端关键测试、前端 typecheck/chunk 检查、服务器 SSH/Compose/health、Cloudflare health 均通过，服务器已预拉取 `postgres:16-alpine`。R1/R2 本地实现已完成：Compose PostgreSQL、database health、备份/恢复脚本和 health 测试已落地，业务读写仍默认 JSON。`video_studio_tasks`、`studio_tasks`、`projects`、media metadata、`project_entities` 和 benchmark records 均已完成 schema/repository、backfill/reconcile、runtime dual-write、read-switch/JSON fallback 和 PostgreSQL primary-write/JSON archive mirror；`project_entities` 覆盖角色、场景、道具、首帧、视频和风格，benchmark records 覆盖图片/视频测评 dataset、suite 和 run。R35-R39 已完成 user/config schema/repository、backfill/reconcile、runtime dual-write、read-switch/JSON fallback 和 PostgreSQL primary-write/JSON archive mirror；R59-R63 已补 `sessions` schema/migration、只保存 `token_hash` 的 repository boundary、脱敏 backfill/reconcile 基础、可选 runtime dual-write、read-switch 和 primary-write/JSON archive mirror，默认运行态仍继续 Redis + file fallback。R48 已完成本地临时 Compose PostgreSQL 实库演练，覆盖 Alembic、全域 backfill/reconcile、备份和恢复；R43 已完成服务器 live migration/backfill/reconcile 和备份/恢复演练；R44 中断在新镜像 build/SSH banner 恢复阶段，未重启容器也未启用业务开关；R45/R46/R49/R50/R51/R52/R53 已补可重复服务器 canary 脚本、app-free 本地 verifier、read-switch/rollback、primary-write/rollback 门禁、全序列 runner、本地 connectivity preflight 和一键远程编排；R54 已新增 `./run.sh doctor` 部署前只读自检；R55/R65 已为 connectivity preflight 补齐 remediation 输出；R56/R58/R59/R65 复测显示当前 Clash DIRECT/IP-CIDR 规则仍未作用到命令行路径；R57 已新增 `MIEMIE_PREFLIGHT_SCOPE=network` 快速门禁，用于先确认 DNS 与 route 干净再运行完整 preflight；R58 已新增 `scripts/pre_studio_server_postgres_sequence.sh` 服务器终端自运行后备入口；R64 已新增 `scripts/postgres_staging_live_data_gate.sh` 并把它插入 staging sequence；R65 已让服务器 fallback 显式校验 `live-data-gate` 存在；R66 已将本机 remote wrapper 对齐为远端同步后调用同一 server fallback。默认运行态仍为 file-only，下一步优先在服务器 `/opt/miemie-pre` 内执行 `CONFIRM_SERVER_SEQUENCE=run scripts/pre_studio_server_postgres_sequence.sh`，或在本机 network/full preflight 均退出 `0` 后继续用 `CONFIRM_REMOTE_SEQUENCE=run scripts/pre_studio_remote_postgres_sequence.sh` 逐级进入 live data gate、staging 小域灰度和回滚验证。
...
2026-06-17 补充：R70 已完成 `audio_studio` runtime dual-write。显式开启 `MIEMIE_DATABASE_DUAL_WRITE_DOMAINS=audio_studio` 或全局 dual-write 后，音频任务与音色档案保存/删除会在 JSON 主路径成功后 shadow 写 PostgreSQL；默认运行态仍为 JSON/file-only，服务器业务开关未启用。下一本地步骤为 R71 read-switch + JSON fallback；证据归档到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r70-audio-studio-runtime-dual-write/`。

...
└── users/
    ...
    │   ├── gallery/        # 图库数据
    │   ├── studio/         # 图片工作室数据
    │   ├── image_benchmark_datasets/ # 图片测评数据集
    │   ├── image_benchmark_suites/   # 图片测评任务配置
    │   ├── image_benchmark_runs/     # 图片测评运行记录
...
图片测评运行 (`POST /image-benchmark/suites/{id}/run`) 同样采用后台异步执行：

1. 创建 `ImageBenchmarkRun` 快照，冻结当次数据集、模型和参数
2. 后台按 case × model 生成单元任务，并用全局/模型级 semaphore 控制并发
3. 单元结果写入 `cell_results`，包含输出图、错误、canonical request、provider payload、task ids、request ids
4. 运行结束后更新 `stats`，前端轮询 `GET /image-benchmark/runs/{id}` 展示矩阵
...
`capabilities -> task_kind -> provider/model -> adapter`

关键点：
- 前端通过 `/api/video-studio/capabilities` 获取任务能力、模型、参数和帮助信息
- 前端主入口按“先选能力，再选模型”工作
- 后端通过 `video_adapters.py` 按 `provider` 分发到 Wan / Kling / Vidu
- 任务 canonical 字段统一保存：
  - `task_kind`
  - `provider`
  - `model_id`
...
参数帮助由后端 schema 统一下发：
- 参数级：`parameter.help`
- 素材位：`ui_hints.asset_help`
- Prompt：`ui_hints.prompt_help`

前端统一使用 `HoverInfoPopover` 渲染结构化帮助，避免模型参数说明散落在多个页面分支里重复维护。

...
- `ratio`: `16:9` / `9:16` / `1:1` / `4:3` / `3:4` / `4:5` / `5:4`，默认 `16:9`
...
- 图片支持公网 URL 或 Base64 data URI，格式 `JPEG/JPG/PNG/WEBP`，宽高均不小于 300 像素，宽高比 `1:2.5~2.5:1`，文件不超过 20MB
...
- 参数支持 `resolution`、`ratio`、`duration`、`watermark`、`prompt_extend`、`disable_data_inspection`、`seed`
- `ratio`: `16:9` / `9:16` / `1:1` / `4:3` / `3:4` / `4:5` / `5:4`，默认 `16:9`
- 参考图支持公网 URL 或 Base64 data URI，格式 `JPEG/JPG/PNG/WEBP`，短边不低于 400 像素，文件不超过 20MB
...
- 参考图支持公网 URL 或 Base64 data URI，格式 `JPEG/JPG/PNG/WEBP`，宽高均不小于 300 像素，宽高比 `1:2.5~2.5:1`，文件不超过 20MB
...
- `/api/video-studio/capabilities` 为 4 个 HappyHorse 任务 profile 暴露高级开关 `prompt_extend=true` 与 `disable_data_inspection=false`。
...
- `happyhorse-1.0-t2v` / `happyhorse-1.0-r2v` capability 和后端 adapter 接受 `4:5`、`5:4` 画幅。

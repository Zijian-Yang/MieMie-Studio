...
当前状态：本地验证通过；pre 服务器已部署到 `7f736affd91a503dd007580af335b0254f3cceb4`，health/首页/Celery、无 key 失败路径、`worker-video` restart 基础恢复和 1 个真实 DashScope 视频 smoke 已归档。真实视频 smoke 的外层 SSH 连接曾中断，恢复后从服务器任务文件核验最终 `succeeded`，并确认测试用户 key 与临时 key 文件已清理。

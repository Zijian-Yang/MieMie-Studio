...
-   [**选择模型**](https://help.aliyun.com/zh/model-studio/use-video-generation#6be7a1bdddpgj)：确认模型所属的地域。
...
| **usage** `*object*` 输出信息统计，只对成功的结果计数。 **属性** **duration** `*integer*` 总的视频计费时长（秒）。示例值：5。 **output\\_video\\_duration** `*integer*` 输出视频的时长（秒）。示例值：5。 **size** `*string*` 输出视频的分辨率。示例值：988\\*932。 **fps** `*integer*` 输出视频的帧率。示例值：24。 **SR** `*string*` 输出视频的分辨率档位。示例值：720。 **audio** `*boolean*` 输出视频是否为有声视频。示例值：false。 **video\\_count** `*integer*` 输出视频的数量。固定为1。 |
| **request\\_id** `*string*` 请求唯一标识。可用于请求明细溯源和问题排查。 |

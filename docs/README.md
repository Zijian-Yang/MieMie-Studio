...
- E2E smoke：`npm run test:e2e`（2026-06-05，6 passed，覆盖登录/未登录跳转、项目列表、旧版视频页迁退提示、视频工作室空态和成功任务详情；macOS 可自动发现本机 `ms-playwright` Chromium 缓存）
...
- 当前已消除：
  ...
  - 项目列表、视频工作室任务列表与详情弹窗缺少 smoke 护栏：`frontend/e2e/smoke.spec.ts` 已补项目列表样本和视频工作室成功任务样本，覆盖项目卡片/统计/入口，以及视频任务列表卡片与详情弹窗主要渲染路径
- 当前保留为后续治理：
  - CI / 服务器环境仍需显式安装 Playwright Chromium；当前自动发现主要覆盖本机已有缓存的开发场景
  - `VideoStudioPage.tsx` 已完成任务展示纯工具函数、数据加载 hook、任务列表组件、任务详情弹窗与任务操作 hook 五刀拆分，后续继续提取创建/编辑表单组件；前端 smoke 仍需补至少一个工作室创建流程；`StudioPage.tsx` 等大页面也仍需继续做行为保持型拆分

...
- Compose PostgreSQL 迁移仍默认 file-only；9 个 tracked 核心业务状态域已完成本地域 schema/repository/backfill/reconcile/runtime gates，coverage pending domain 为 0；R88 已补齐 `UserService.list_user_ids()` 的 PostgreSQL 读路径，避免最终模式下视频任务恢复枚举用户仍固定依赖根级 `users.json`；下一步仍是服务器 final exit sequence 和后置验证，不是直接宣称完成切库
- 当前主风险不在“代码完全不可用”，而在：
  - 复杂页面/路由/服务文件过大
  - 前端自动化测试缺口明显
  - 主题 token 与文档约束存在回潮

...
- Compose 也会定义 PostgreSQL：
  ...
  - 最终 JSON exit 前必须先运行 `CONFIRM_APPLY_FINAL_JSON_EXIT_POLICY=run SEQUENCE_ARTIFACT_DIR=<server-sequence-artifact> scripts/postgres_apply_final_json_exit_policy.sh`。该脚本会备份 `compose.env`、写入最终 PostgreSQL-only 运行态策略，并复跑 final JSON exit audit。

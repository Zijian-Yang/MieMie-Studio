...
- Compose 也会定义 PostgreSQL：
  ...
  - 最终 JSON exit 推荐使用服务器总入口：`CONFIRM_SERVER_FINAL_EXIT_SEQUENCE=run scripts/pre_studio_server_postgres_final_exit_sequence.sh`。该入口会先执行服务器 sequence，再应用最终 PostgreSQL-only 策略，随后执行 post JSON exit validation；若最终策略应用或后置验证失败，默认尝试用 R82 env 备份自动回滚。
  - 最终完成判定使用 `python3 scripts/postgres_final_exit_completion_audit.py --final-exit-artifact-dir <server-final-exit-artifact>`。只有该审计输出 `postgres_only_complete`，才可宣称平台运行态已经完全脱离 JSON 主/备读写；历史 JSON 文件归档或删除是后续单独维护动作。
  - 历史 tracked 业务 JSON 归档使用 `python3 scripts/postgres_archive_json_after_final_exit.py --completion-status <completion-status.json> --confirm archive`。该脚本会先确认 completion status 为 `postgres_only_complete`，再为 sessions、用户配置、项目、工作室任务、媒体元数据、项目实体、benchmark 和 audio studio JSON 创建 tar.gz 并移动到 quarantine；根级 `config.json`、`users.json`、`config.example.json` 不在归档范围。
  - 如需拆步执行，最终 JSON exit 前必须先运行 `CONFIRM_APPLY_FINAL_JSON_EXIT_POLICY=run SEQUENCE_ARTIFACT_DIR=<server-sequence-artifact> scripts/postgres_apply_final_json_exit_policy.sh`。该脚本会备份 `compose.env`、写入最终 PostgreSQL-only 运行态策略，并复跑 final JSON exit audit。
  - 也可只读运行 `python3 scripts/postgres_final_json_exit_audit.py --sequence-artifact-dir <server-sequence-artifact> --env-file compose.env`。只有该审计进入 `ready_for_post_json_exit_validation` 后，才继续跑后置 health、全域 reconcile 和保守负载门禁。
  - 后置门禁使用 `CONFIRM_POST_JSON_EXIT_VALIDATION=run SEQUENCE_ARTIFACT_DIR=<server-sequence-artifact> scripts/postgres_post_json_exit_validation.sh`。该脚本会复查 final JSON exit audit、滚动最终运行态、检查本机/公网 health、跑全域 reconcile、采集 Docker 状态并执行 k6 S1 读负载。

...
const baseUrl = (__ENV.MIEMIE_BASE_URL || 'http://127.0.0.1:18100').replace(/\/$/, '')
const authToken = (__ENV.MIEMIE_AUTH_TOKEN || '').trim()
const loadtestRunId = (__ENV.LOADTEST_RUN_ID || 'w2-300-entry-comparison')
const scenarioName = (__ENV.SCENARIO_NAME || 'entry-status-300')
const forcedHostIp = (__ENV.MIEMIE_FORCE_HOST_IP || '').trim()
const insecureSkipTLSVerify = (__ENV.MIEMIE_INSECURE_SKIP_TLS_VERIFY || '') === '1'
...
const hosts = forcedHostIp ? { 'pre-studio.miemie.co': forcedHostIp } : {}

export const options = {
  vus: Number(__ENV.K6_VUS || 300),

...
- 下一步优先级：新 Mac 或新服务器先跑 `./run.sh doctor`，Compose 路径补 `DOCTOR_PROFILE=compose ./run.sh doctor`；数据库灰度恢复有两条路：若继续从本机执行，先用 `MIEMIE_PREFLIGHT_SCOPE=network scripts/pre_studio_connectivity_preflight.sh` 快速确认 DNS 与 route 干净，再让完整 preflight 退出 `0`，最后运行 `CONFIRM_REMOTE_SEQUENCE=run scripts/pre_studio_remote_postgres_sequence.sh`；若本机 Clash/TUN 仍阻塞，则直接在服务器 `/opt/miemie-pre` 中运行 `CONFIRM_SERVER_SEQUENCE=run scripts/pre_studio_server_postgres_sequence.sh`。两条路最终都会串行执行审计、运行态滚动、live data gate、双写、读切换、回滚、主写和主写回滚门禁，随后补保守查询门禁。R45/R46/R49/R50/R51/R52/R53 已把本机远程路径固化成脚本和 verifier，R54 补齐部署环境自检，R55/R65 补齐 blocked 时的 remediation 输出，R56/R58/R65 证明当前 Clash DIRECT/IP-CIDR 规则尚未作用到命令行路径，R57 增加快速 network scope，R58 增加服务器终端自运行后备入口，R64/R65 补齐 app-level canary 前的服务器 live data gate 与 fallback 契约，R66 将本机 remote wrapper 对齐到同一 server fallback，R67 明确下一本地迁移域为 `audio_studio`，R68-R72 已完成该域 schema/repository/backfill/reconcile/runtime dual-write/read-switch/primary-write；R48 已补齐本地全域实库演练通过证据。应用 500 已清零，Cloudflare 100 VU 已恢复通过，300 VU 主要瓶颈已收窄到源站公网/Cloudflare 边缘链路。本地跨境直连与美国代理样本均已补齐为风险记录。W2 平台侧阶段可收口；阶段 7 已完成服务器 live migration/backfill/reconcile，但尚未完成新版 Alembic/session/audio 域 live data gate、业务开关灰度和最终切库；若服务器命令路径继续阻塞，下一步先跑新的 coverage audit 确认是否还有非核心 JSON-only 状态，否则回到服务器 live-data/canary 灰度。

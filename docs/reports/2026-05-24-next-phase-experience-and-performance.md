...
- 下一步优先级：新 Mac 或新服务器先跑 `./run.sh doctor`，Compose 路径补 `DOCTOR_PROFILE=compose ./run.sh doctor`；数据库灰度恢复时先让 `scripts/pre_studio_connectivity_preflight.sh` 退出 `0`，再运行 `CONFIRM_REMOTE_SEQUENCE=run scripts/pre_studio_remote_postgres_sequence.sh`，由 wrapper 同步服务器 repo 并串行执行审计、运行态滚动、双写、读切换、回滚、主写和主写回滚门禁，随后补 reconcile 和保守查询门禁。R45/R46/R49/R50/R51/R52/R53 已把这条恢复路径固化成脚本和本地 verifier，R54 补齐部署环境自检，R55 补齐 blocked 时的 remediation 输出；R48 已补齐本地全域实库演练通过证据。应用 500 已清零，Cloudflare 100 VU 已恢复通过，300 VU 主要瓶颈已收窄到源站公网/Cloudflare 边缘链路。本地跨境直连与美国代理样本均已补齐为风险记录。W2 平台侧阶段可收口；阶段 7 已完成服务器 live migration/backfill/reconcile，但尚未完成业务开关灰度和最终切库。

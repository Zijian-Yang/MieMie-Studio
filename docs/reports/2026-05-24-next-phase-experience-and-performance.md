...
2026-06-18 阶段 7 R86 final exit completion audit：

- 新增 `scripts/postgres_final_exit_completion_audit.py` 与 `scripts/verify_postgres_final_exit_completion_audit.py`，把“最终完全脱离 JSON”的完成判定落到只读 artifact 审计。
- 该审计要求服务器最终 exit、server sequence wrapper、inner sequence、最终策略应用、post JSON exit validation 全部通过，并确认回滚没有通过；只有全部满足才输出 `postgres_only_complete`。
- 本地验证：先观察 verifier 因缺少脚本失败，随后实现脚本并通过 verifier。对 R84 dry-run artifact 运行 R86 审计后状态为 `needs_final_exit_evidence`，证明当前仍不能宣称最终脱离 JSON。

2026-06-18 阶段 7 R87 post-final JSON archive gate：

- 新增 `scripts/postgres_archive_json_after_final_exit.py` 与 `scripts/verify_postgres_archive_json_after_final_exit.py`，为最终完成后的 tracked 业务 JSON 归档/隔离建立门禁。
...
2026-06-18 阶段 7 R91 network preflight after fake-ip off：

- 用户关闭 Clash 订阅配置 fake-ip 后复跑 network-scope preflight，状态仍为 `blocked`。
...
2026-06-19 阶段 7 R99-R103 PostgreSQL final exit：

- 服务器 SSH 通道恢复后，`/opt/miemie-pre` 先后快进到 `492c346`、`6059dac`、`209217b` 和 `3444161`，并执行多轮 final exit sequence。
- R99 暴露全域 canary 在 `-w /app/backend` 下写错 `user_config` JSON 路径；已修正为 `UserService.get_user_data_path(user.id)`，并补 verifier。
- R100 暴露 R99 留下的 PostgreSQL-only canary config 残留；已新增 `scripts/postgres_cleanup_canary_user_config_residue.py` 并插入 live-data gate，只清理 `canary_*` 且 PostgreSQL 有 config、JSON 无 config 的测试残留。
- R101 数据 reconcile、全域 canary 和最终策略应用均通过，但 post JSON exit k6 S1 read gate 失败：`http_req_failed=0`，P95 `471.68ms` 超过 `300ms`。根因是 `/api/health` 每次请求新建 SQLAlchemy engine、连接 PostgreSQL 后立即 dispose，在 30 VU 并发 health gate 下放大尾延迟。
- R102 修复 `/api/health` 数据库探针，按 URL/超时复用 health 专用 SQLAlchemy engine。最终服务器 sequence、最终 PostgreSQL-only policy、post JSON exit validation 全部通过；k6 S1 read gate 为 `2600` requests、失败率 `0`、P95 `65.84ms`、P99 `111.42ms`。
...
2026-06-20 阶段 7 R118 operational alert hook：
...
2026-06-20 阶段 7 R120 database operational snapshot：
...
2026-06-20 阶段 7 R121 database snapshot cron integration：

- `scripts/postgres_install_operational_cron.sh` 新增第三条默认计划：每日 `05:15` 运行 `CONFIRM_POSTGRES_DATABASE_SNAPSHOT=run bash scripts/postgres_database_snapshot.sh`，日志写入 `logs/postgres-database-snapshot-cron.log`。
...
2026-06-20 阶段 7 R123 cron evidence trigger source：

- 为 PostgreSQL 运维 artifact 增加来源标记：cron 安装器生成的三条定时任务会传 `POSTGRES_OPS_TRIGGER=cron`，手动 sequence 会传 `POSTGRES_OPS_TRIGGER=manual_sequence`；readiness、backup retention 和 database snapshot 的 `status.json` 均记录 `trigger`。
- `scripts/postgres_operational_cron_evidence.sh` 新增 `CRON_EVIDENCE_REQUIRED_TRIGGER`。后续检查“自然 cron 首跑”时应设置 `CRON_EVIDENCE_REQUIRED_TRIGGER=cron`，避免 R122 或后续手动 sequence artifact 被误当作自然定时证据。
- 本地 verifier 与 dry-run 已通过，证据归档到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r123-postgres-cron-trigger-source-dry-run-20260620/`、`r123-postgres-cron-evidence-trigger-filter-dry-run-20260620/` 和 `r123-postgres-cron-sequence-trigger-dry-run-20260620/`。服务器 cron 已刷新到带 `trigger=cron` 的版本，安装状态 `passed`；立即用 `CRON_EVIDENCE_REQUIRED_TRIGGER=cron` 复查时为预期 `waiting`，原因是新版自然 cron artifact 尚未生成。服务器证据归档到 `docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r123-postgres-cron-trigger-source-refresh-20260620/` 和 `r123-postgres-cron-trigger-source-evidence-current-20260620/`。
...
2026-06-23 阶段 7 R125 pre-studio 入口审计：

- 新增 `scripts/pre_studio_entrypoint_audit.py` 与 `scripts/verify_pre_studio_entrypoint_audit.py`，把 Cloudflare/Nginx/API/静态资源入口检查固化为默认 dry-run、显式执行的只读门禁。
- 本地公网审计通过：`/api/health` 为 `200`，`status=ok`，`redis.ok=true`，`database.ok=true`，`server=cloudflare`，`cf-cache-status=DYNAMIC`，`cache-control=no-store`；首页引用的 `/_static/index-D-1V5NKu.js` 二次请求为 `cf-cache-status=HIT`，静态缓存为 `public, max-age=604800, immutable`。
- 服务器同机审计通过：额外验证 `LOCAL_BASE_URL=http://127.0.0.1:18100` 的 origin health 为 `200` 且 Redis/PostgreSQL 均正常；本机 origin 未返回 `cache-control=no-store` 已降级为 warning，公网 API 仍作为硬门禁要求 no-store。

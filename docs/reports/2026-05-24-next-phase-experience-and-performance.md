...
- 下一步优先级：新 Mac 或新服务器先跑 `./run.sh doctor`，Compose 路径补 `DOCTOR_PROFILE=compose ./run.sh doctor`；数据库灰度恢复有两条路：若继续从本机执行，先用 `MIEMIE_PREFLIGHT_SCOPE=network scripts/pre_studio_connectivity_preflight.sh` 快速确认 DNS 与 route 干净，再让完整 preflight 退出 `0`，最后运行 `CONFIRM_REMOTE_SEQUENCE=run scripts/pre_studio_remote_postgres_sequence.sh`；若本机 Clash/TUN 仍阻塞，则直接在服务器 `/opt/miemie-pre` 中运行 `CONFIRM_SERVER_SEQUENCE=run scripts/pre_studio_server_postgres_sequence.sh`。两条路最终都会串行执行审计、运行态滚动、live data gate、全域双写、全域读切换、读回滚、全域主写和主写回滚门禁，随后补保守查询门禁。R45/R46/R49/R50/R51/R52/R53 已把本机远程路径固化成脚本和 verifier，R54 补齐部署环境自检，R55/R65 补齐 blocked 时的 remediation 输出，R56/R58/R65/R74/R77/R79/R80 证明当前 Clash DIRECT/IP-CIDR 规则尚未作用到命令行路径，R57 增加快速 network scope，R58 增加服务器终端自运行后备入口，R64/R65 补齐 app-level canary 前的服务器 live data gate 与 fallback 契约，R66 将本机 remote wrapper 对齐到同一 server fallback，R68-R72 已完成 `audio_studio` 本地迁移闭环，R73 已确认 9 个 tracked 核心业务状态域 pending 为 0，R75 已确认最终切库前还缺全域 provider-free app canary，R76 已补齐全域 canary 契约并把 readiness 推进到 `ready_for_final_cutover_sequence`，R78 已补强服务器 fallback 执行前全域契约校验，R80 已补齐 final JSON exit audit；R48 已补齐本地全域实库演练通过证据。应用 500 已清零，Cloudflare 100 VU 已恢复通过，300 VU 主要瓶颈已收窄到源站公网/Cloudflare 边缘链路。本地跨境直连与美国代理样本均已补齐为风险记录。W2 平台侧阶段可收口；阶段 7 已完成服务器 live migration/backfill/reconcile，但尚未完成新版 Alembic/session/audio 域 live data gate、全域 app canary 实跑、业务开关灰度和最终切库；下一步仍需服务器终端 fallback 或清理本机 TUN/fake-IP 后执行 sequence。

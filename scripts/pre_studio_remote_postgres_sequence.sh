...
RUN_ID="${RUN_ID:-pre-studio-remote-postgres-sequence-$(date +%Y%m%d%H%M%S)}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$ROOT_DIR/docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r53-remote-postgres-sequence}"
  ...
  cat > "$STATUS_FILE" <<JSON
{
  ...
  "confirm": "$(json_escape "$CONFIRM_REMOTE_SEQUENCE")",
...
build_remote_command() {
  local quoted_remote_dir quoted_branch quoted_run_id quoted_artifact_dir quoted_tmp_dir quoted_runner
...
run_remote_sequence() {
  ...
  ssh -o BatchMode=yes -o ConnectTimeout="$CONNECT_TIMEOUT" -o StrictHostKeyChecking=accept-new -p "$SSH_PORT" "$SSH_TARGET" "$remote_command" \
    > "$ARTIFACT_DIR/remote-sequence.out" 2>"$ARTIFACT_DIR/remote-sequence.err"
  exit_code=$?
  set -e
  if [[ "$exit_code" != "0" ]]; then
    write_status "failed" "remote-sequence" "remote sequence exited with $exit_code"
    return "$exit_code"
  fi
}
...
main() {
  build_remote_command
  if [[ "$CONFIRM_REMOTE_SEQUENCE" != "run" ]]; then
    write_status "dry_run" "planned" "set CONFIRM_REMOTE_SEQUENCE=run to execute local preflight and remote sequence"
    log_line "dry-run only; execution skipped"
    printf 'dry-run remote command written to %s\n' "$REMOTE_COMMAND_FILE"
    return 0
  fi

  write_status "running" "preflight" ""
  run_local_preflight
  write_status "running" "remote-sequence" ""
  run_remote_sequence
  pull_remote_artifacts

#!/usr/bin/env python3
"""Backfill per-user benchmark JSON files into PostgreSQL."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill image/video benchmark records from JSON files into PostgreSQL."
    )
    ...
    parser.add_argument(
        ...
        default=str(
            REPO_ROOT
            / "docs/reports/artifacts/2026-06-07-postgres-upgrade-rollout/r31-benchmark-records-backfill-reconcile/benchmark_records_backfill.json"
        ),
    )
...
def main() -> int:
    ...
    try:
        summary = backfill_benchmark_records(args.data_root, repository_factory)

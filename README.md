...
- **图片尺寸跟随策略**：`qwen-image-2.0-pro / qwen-image-2.0` 在图片编辑场景支持“不设置尺寸（跟随最后一张输入图分辨率）”；`wan2.7-image` 系列默认 2K，输入图场景下输出比例跟随最后一张图
- **图片开发者模式**：图片工作室新建任务和任务详情都支持查看 canonical 请求体、厂商 payload、task id、request id 和结果元信息
...
- **测评追踪信息**：图片测评单元详情留存 canonical 请求体、厂商 payload、所有 task ids、所有 request ids 和结果元信息，便于厂商工单与线上排查
...
- **任务通知与封面**：视频任务完成后可触发浏览器通知；成功任务会自动从输出视频抽取首帧作为任务封面
- [ ] **添加导入/导出用户资产功能**

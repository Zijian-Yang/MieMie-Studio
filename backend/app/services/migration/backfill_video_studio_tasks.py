"""Backfill video studio task JSON files into PostgreSQL repositories."""
...
from app.repositories.base import VideoStudioTaskRepository


RepositoryFactory = Callable[[str], VideoStudioTaskRepository]


@dataclass(frozen=True)
class VideoStudioJsonTaskRecord:
    user_id: str
    task: VideoStudioTask
    source_path: Path


def iter_video_studio_json_tasks(
    data_root: str | Path,
    *,
    on_error: Optional[Callable[[str, Path, Exception], None]] = None,
) -> Iterable[VideoStudioJsonTaskRecord]:
    """Yield valid per-user video studio tasks from `data/users/*/video_studio`."""
    ...
    for user_dir in sorted(path for path in users_dir.iterdir() if path.is_dir()):
        task_dir = user_dir / "video_studio"
        ...
        for task_path in sorted(task_dir.glob("*.json")):
            try:
                ...
                yield VideoStudioJsonTaskRecord(
                    user_id=user_dir.name,
                    task=VideoStudioTask(**data),
...
def backfill_video_studio_tasks(
    data_root: str | Path,
    repository_factory: RepositoryFactory,
) -> dict:
    """Upsert all valid per-user JSON video studio tasks into PostgreSQL."""
    ...
    for record in iter_video_studio_json_tasks(data_root, on_error=record_load_failure):

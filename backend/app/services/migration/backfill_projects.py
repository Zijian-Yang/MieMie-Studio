"""Backfill project JSON files into PostgreSQL repositories."""
...
def iter_project_json_files(
    ...
    *,
...
) -> Iterable[ProjectJsonRecord]:
    """Yield valid per-user projects from `data/users/*/projects`."""
    ...
    for user_dir in sorted(path for path in users_dir.iterdir() if path.is_dir()):
        project_dir = user_dir / "projects"
        if not project_dir.exists():
            continue
        for project_path in sorted(project_dir.glob("*.json")):
            try:
                with project_path.open("r", encoding="utf-8") as handle:
                    data = json.load(handle)
                yield ProjectJsonRecord(
                ...
                if on_error:
                    on_error(user_dir.name, project_path, exc)
...
def backfill_projects(
    ...
    """Upsert all valid per-user JSON projects into PostgreSQL."""
    ...
    for record in iter_project_json_files(data_root, on_error=record_load_failure):
        ...
        json_count += 1
        try:
            repository_factory(record.user_id).save(record.project)
            upserted_count += 1
        except Exception as exc:
            failures.append(
                {

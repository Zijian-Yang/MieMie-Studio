...
def _enable_dual_write(monkeypatch):
    ...
    monkeypatch.setenv("MIEMIE_DATABASE_DUAL_WRITE_DOMAINS", "video_studio_tasks")
    monkeypatch.setenv("MIEMIE_DATABASE_WRITE_MODE", "file")
    monkeypatch.setenv("MIEMIE_DATABASE_RECONCILE_STRICT", "false")


def test_video_studio_task_dual_write_is_disabled_by_default(tmp_path, monkeypatch):
    shadow = _ShadowRepository()
    monkeypatch.setattr(
        "app.repositories.video_studio_task_runtime.build_video_studio_task_shadow_repository",
    ...
    assert storage.get_video_studio_task("task-1") is None


def test_video_studio_task_dual_write_saves_and_marks_deleted_when_enabled(tmp_path, monkeypatch):
...
def test_video_studio_task_dual_write_failure_does_not_break_json_primary(tmp_path, monkeypatch):
    _enable_dual_write(monkeypatch)

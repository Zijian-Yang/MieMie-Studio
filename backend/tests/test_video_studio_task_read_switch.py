...
def _enable_read_switch(monkeypatch, *, fallback=True):
    ...
    monkeypatch.setenv("MIEMIE_DATABASE_READ_DOMAINS", "video_studio_tasks")
...
def test_video_studio_task_reads_are_file_only_by_default(tmp_path, monkeypatch):
    repo = _ReadRepository([_task("pg-task")])
    monkeypatch.setattr(
        "app.repositories.video_studio_task_runtime.build_video_studio_task_read_repository",
        lambda user_id: repo,
    )
    storage = StorageService(str(tmp_path), owner_user_id="user-1")
    storage.save_video_studio_task(_task("json-task"))

    assert storage.get_video_studio_task("json-task").id == "json-task"
    assert storage.get_video_studio_tasks("project-1")[0].id == "json-task"
    assert storage.get_all_video_studio_tasks()[0].id == "json-task"
    assert repo.get_calls == []
    assert repo.project_calls == []
    assert repo.all_calls == 0


def test_video_studio_task_read_switch_uses_postgres_for_get_and_lists(tmp_path, monkeypatch):
    ...
    assert storage.get_video_studio_task("pg-newer").id == "pg-newer"
    assert [task.id for task in storage.get_video_studio_tasks("project-1")] == [
        "pg-newer",
        "pg-older",
    ]
    assert [task.id for task in storage.get_all_video_studio_tasks()] == [
        "pg-newer",
        "pg-other",
        "pg-older",
    ]
...
def test_video_studio_task_read_switch_falls_back_to_json_on_postgres_miss(tmp_path, monkeypatch):
    _enable_read_switch(monkeypatch)
    ...
    assert storage.get_video_studio_task("json-task").id == "json-task"
    assert [task.id for task in storage.get_video_studio_tasks("project-1")] == ["json-task"]
    assert [task.id for task in storage.get_all_video_studio_tasks()] == ["json-task"]


def test_video_studio_task_read_switch_raises_when_fallback_disabled(tmp_path, monkeypatch):
    ...
    try:
        storage.get_video_studio_task("json-task")

  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, margin: 0, color: token.colorText }}>场景管理</h1>
        ...
        <Space>
          ...
          {scenes.length > 0 && (
            <>
              ...
              <Popconfirm title="确定删除所有场景？" description="此操作不可恢复" icon={<ExclamationCircleOutlined style={{ color: token.colorError }} />} onConfirm={deleteAllScenes} okText="删除" cancelText="取消" okButtonProps={{ danger: true }}>
            ...
            return (
              <div key={scene.id} className="asset-card" onClick={() => openSceneModal(scene)} style={{ opacity: isGeneratingThis ? 0.7 : 1 }}>
                <div className="asset-card-image" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  {thumbnailUrl ? <Image src={thumbnailUrl} alt={scene.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} preview={false} /> : <PictureOutlined style={{ fontSize: 48, color: token.colorTextQuaternary }} />}
                ...
                return (
                  <div key={groupIndex} style={{ marginBottom: 12, padding: 12, border: isSelected ? `2px solid ${token.colorWarning}` : `1px solid ${token.colorBorder}`, borderRadius: 8, background: isSelected ? token.colorFillQuaternary : token.colorBgLayout }}>
                    ...
                    <div style={{ aspectRatio: '16/9', background: token.colorBgContainer, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {isGeneratingThis ? <Spin /> : group?.url ? <Image src={group.url} alt="场景" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <PictureOutlined style={{ fontSize: 32, color: token.colorTextQuaternary }} />}
                    </div>
                  </div>
                )
              })}

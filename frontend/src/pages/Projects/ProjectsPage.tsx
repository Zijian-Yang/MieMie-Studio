  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 600, margin: 0, color: token.colorText }}>
            项目列表
          </h1>
      ...
      {loading ? (
        ...
        <Empty description="暂无项目" style={{ marginTop: 100 }}>
          <Button type="primary" onClick={() => setIsModalOpen(true)}>创建第一个项目</Button>
        </Empty>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
          {projects.map((project) => (
            <Card
              ...
              hoverable
              actions={[
                <Button key="open" type="text" icon={<FolderOpenOutlined />} onClick={() => handleOpenProject(project.id)}>打开</Button>,
                <Popconfirm key="delete" title="确定要删除这个项目吗？" description="删除后将无法恢复" onConfirm={() => handleDeleteProject(project.id)} okText="删除" cancelText="取消" okButtonProps={{ danger: true }}>
                  <Button type="text" danger icon={<DeleteOutlined />}>删除</Button>
                </Popconfirm>,
              ]}
            >
              <Card.Meta
                title={<span style={{ color: token.colorText, fontSize: 16 }}>{project.name}</span>}
                description={
                  <div>
                    <p style={{ color: token.colorTextSecondary, marginBottom: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      ...
      <Modal title="新建项目" open={isModalOpen} onOk={handleCreateProject} onCancel={() => { setIsModalOpen(false); form.resetFields() }} okText="创建" cancelText="取消">
